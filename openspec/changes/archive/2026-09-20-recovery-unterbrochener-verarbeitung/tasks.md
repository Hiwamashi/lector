...
- [x] 1.1 Abfrage in `app/repository.py` ergänzen, die alle Vorgänge im Zustand `processing`
...
- [x] 1.2 Hilfsfunktion ergänzen, die zu einem Vorgang bestimmt, wo sein Original liegt
...
- [x] 2.1 Auflösung nach der Entscheidungstabelle aus `design.md` (D1) implementieren, ohne
...
- [x] 2.2 Grenzfall aus D2 ergänzen: Original im Eingang, kein Ablageort vermerkt, aber eine
...
- [x] 2.3 Abschluss über denselben Weg wie im Normalfall (D4): Original nachziehen, wenn es
      ...
      (`skipped_erechnung`)
- [x] 2.4 Verlaufseintrag je aufgelöstem Vorgang schreiben, der die Unterbrechung und die
      getroffene Auflösung benennt — verifiziert durch einen Test, der den Verlauf nach der
      Auflösung ausliest
- [x] 2.5 Fehler je Vorgang einzeln abfangen und protokollieren, ohne die Auflösung der
...
- [x] 3.1 Auflösung in `lifespan` (`app/main.py`) aufrufen, nach dem Öffnen der Datenbank und
...
- [x] 3.2 Anzahl der aufgelösten Vorgänge je Start protokollieren (nur bei mehr als null) —
      verifiziert durch einen Test, der die Protokollausgabe mit `caplog` prüft
- [x] 3.3 Reihenfolge absichern: kein neu aufgenommenes Dokument darf vor der Auflösung
...
- [x] 4.1 Regressionsprobe: Die neuen Tests gegen die *unveränderte* Fassung laufen lassen und
