...
| `RETRY_DELAY_MINUTES` mindestens 1 | immer | bei 0 oder negativ liegt `retry_at` im Jetzt oder in der Vergangenheit (`app/repository.py:331`) — die Pause zwischen den Versuchen entfällt vollständig, und ein dauerhaft scheiterndes Dokument verbraucht alle Versuche in Sekunden, jeden mit einem bezahlten OCR-Aufruf |
